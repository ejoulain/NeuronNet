 ...
 */
    std::pair<size_t, double> degree(const size_t&) const;  //à reécrire
 ...
 */
    std::vector<std::pair<size_t, double> > neighbors(const size_t&) const; //à reécrire
 ...
 */
    std::set<size_t> step(const std::vector<double>&); //à reécrire
